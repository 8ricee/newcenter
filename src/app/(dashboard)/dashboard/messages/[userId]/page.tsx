...
export default async function MessageDetailPage({
    ...
    const { messages, otherUser, error } = await getMessages(params.userId)
            ...
            <div className="p-4 border-b flex items-center space-x-2">
                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                    {otherUser.image ? (
                        ...
                        <span className="text-primary font-medium">{otherUser.name?.charAt(0) || "U"}</span>
                    )}
                </div>
                <div>
                    <h2 className="font-medium">{otherUser.name}</h2>
                    <p className="text-xs text-muted-foreground">
                        {otherUser.role === "TEACHER" ? "Giảng viên" : otherUser.role === "STUDENT" ? "Học viên" : "Quản trị viên"}
                    </p>
                </div>
            </div>
